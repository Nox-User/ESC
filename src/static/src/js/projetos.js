...
// ========= Gráfico ========
function agruparProjetosPorMes(projetos, anos) {
  ...
  projetos.forEach(p => {
    ...
    resultado[indiceMes].projetos += 1;
...
async function carregarDadosBackend() {
  try {
    ...
    projetosanuais = await response.json();
    console.log("Projetos carregados do backend:", projetosanuais);
    
    graphData = agruparProjetosPorMes(projetosanuais);
...
async function carregarStatusData() {
  try {
    const response = await fetch(`${API_BASE_URL}/projetos/status-count?ano=${year}&mes=${monthnumber}`);
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
...
function bindAnoFiltro(){
  ...
  select.addEventListener("change", async () => {
    ...
    try {
      const url = ano ? `${API_BASE_URL}/projetos?ano=${ano}` : `${API_BASE_URL}/projetos`;
      const response = await fetch(url);
      if (response.ok) {
        projetosanuais = await response.json();
        graphData = agruparProjetosPorMes(projetosanuais, ano);
...
function gerarStatusData(projetos, year) {
  const statusBase = [
    { id: 1, name: 'NÃO INICIADO', position: "Quantidade de projetos não iniciados"},
    { id: 2, name: 'EM ANDAMENTO', position: "Quantidade de projetos em andamento"},
    { id: 3, name: 'FINALIZADO', position: "Quantidade de projetos finalizados"},
  ];
  ...
  function mapearStatus(status) {
    ...
    if (s === "NAO INICIADO" || s === "PLANEJAMENTO") return "NÃO INICIADO";
    if (s === "EM DESENVOLVIMENTO" || s === "EM ANDAMENTO" || s === "EXECUÇÃO" || s === "ANÁLISE") return "EM ANDAMENTO";
    if (s === "FINALIZADO" || s === "CONCLUÍDO" || s === "ENTREGUE") return "FINALIZADO";
    return null;
  }
  ...
  function render(){
    mount.innerHTML = `
      <div class="flex-shrink-0 overflow-hidden p-2 mt-12">
        <div class="flex items-center h-full sm:justify-center xl:justify-start p-2 sidebar-separator-top">
          ...
          <div class="block sm:hidden xl:block ml-2 font-bold text-xl text-black">PROJETOS</div>
    ...
    $$('.js-menu').forEach(el=>{
      el.addEventListener('click', ()=>{
        ...
        if (selected === '1') {
          // carrega a página de projetos overview
          ProjetosPage(content);
        } else {
    ...
    <div class="w-full">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center">
          <div class="text-2xl font-bold">Dashboard de Projetos</div>
          <div class="ml-4">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ${statusData.map(item => `
          <div class="bg-white p-6 rounded-lg shadow-md projeto-card">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm text-gray-600">${item.position}</p>
                <p class="text-2xl font-bold text-gray-900">${item.transactions}</p>
                <p class="text-lg font-semibold projeto-status-badge">${item.name}</p>
              </div>
...
// Função para página de projetos (Overview)
function ProjetosPage(mount) {
  mount.innerHTML = `
    <div class="w-full tabela-overview">
      <div class="flex items-center justify-between mb-4">
        <div class="text-2xl font-bold">Lista de Projetos</div>
        <button onclick="adicionarProjeto()" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
          Adicionar Projeto
        </button>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="overflow-x-auto">
          <table class="min-w-full">
            <thead class="projeto-table-header">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Prioridade</th>
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Prazo</th>
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Ações</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              ${projetosanuais.map(projeto => `
                <tr class="projeto-row hover:bg-gray-50">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-medium text-gray-900">${projeto.TITULO || 'Sem título'}</div>
                    <div class="text-sm text-gray-500">${projeto.DESCRICAO || ''}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${projeto.RESPONSAVEL || 'Não definido'}</td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(projeto.STATUS)}">${projeto.STATUS || 'Não definido'}</span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="${getPriorityColor(projeto.PRIORIDADE)}">${projeto.PRIORIDADE || 'Não definida'}</span>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${projeto.PRAZO || 'Não definido'}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button onclick="editarProjeto('${projeto.id}')" class="text-blue-600 hover:text-blue-900 mr-3">Editar</button>
...
function getStatusColor(status) {
  ...
  if (s.includes('FINALIZADO') || s.includes('CONCLUÍDO')) return 'bg-green-100 text-green-800';
...
function getPriorityColor(prioridade) {
  if (!prioridade) return 'text-gray-500';
  const p = prioridade.toUpperCase();
  if (p.includes('ALTA')) return 'projeto-priority-high';
  if (p.includes('MÉDIA') || p.includes('MEDIA')) return 'projeto-priority-medium';
  return 'projeto-priority-low';
}
...
async function adicionarProjeto() {
  ...
  const prioridade = prompt('Prioridade (Alta/Média/Baixa):');
...
async function editarProjeto(id) {
  const projeto = projetosanuais.find(p => p.id === id);
  if (!projeto) return;
  
  const titulo = prompt('Título do projeto:', projeto.TITULO);
  if (titulo === null) return;
  
  const responsavel = prompt('Responsável:', projeto.RESPONSAVEL);
  const status = prompt('Status:', projeto.STATUS);
  const descricao = prompt('Descrição:', projeto.DESCRICAO);
  const prazo = prompt('Prazo (DD/MM/YYYY):', projeto.PRAZO);
  const prioridade = prompt('Prioridade (Alta/Média/Baixa):', projeto.PRIORIDADE);
...
// Componentes de gráfico e outros (simplificados para projetos)
function Graph(mount) {
  mount.innerHTML = `
    <div>
      <h3 class="text-lg font-semibold mb-4">Projetos por Mês</h3>
      <div class="h-64 flex items-end justify-between space-x-2">
        ${graphData.map(item => `
          <div class="flex flex-col items-center">
            <div class="bg-blue-500 rounded-t" style="height: ${Math.max(item.projetos * 10, 5)}px; width: 20px;"></div>
            <div class="text-xs mt-1">${item.name}</div>
            <div class="text-xs text-gray-500">${item.projetos}</div>
          </div>
        `).join('')}
      </div>
    </div>`;
...
function AddComponent(mount) {
  mount.innerHTML = `
    <div>
      ...
      <div class="space-y-3">
        <button onclick="adicionarProjeto()" class="w-full p-3 text-left bg-blue-50 hover:bg-blue-100 rounded-lg border border-blue-200">
          <div class="font-medium text-blue-900">Novo Projeto</div>
          <div class="text-sm text-blue-700">Adicionar um novo projeto ao sistema</div>
        </button>
        <button class="w-full p-3 text-left bg-green-50 hover:bg-green-100 rounded-lg border border-green-200">
          <div class="font-medium text-green-900">Relatório Mensal</div>
          <div class="text-sm text-green-700">Gerar relatório de projetos do mês</div>
        </button>
      </div>
    </div>`;
}
...
function Clientes(mount) {
  mount.innerHTML = `
    <div>
      <h3 class="text-lg font-semibold mb-4">Responsáveis Ativos</h3>
      <div class="space-y-2">
        ${[...new Set(projetosanuais.map(p => p.RESPONSAVEL).filter(r => r))].slice(0, 5).map(responsavel => `
          <div class="flex items-center justify-between p-2 bg-gray-50 rounded">
            <span class="text-sm">${responsavel}</span>
            <span class="text-xs text-gray-500">${projetosanuais.filter(p => p.RESPONSAVEL === responsavel).length} projetos</span>
          </div>
        `).join('')}
      </div>
    </div>`;
...
function Satisfaction(mount) {
  mount.innerHTML = `
    <div>
      <h3 class="text-lg font-semibold mb-4">Status dos Projetos</h3>
      <div class="space-y-3">
        ${statusData.map(item => `
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center">
              <div class="w-20 bg-gray-200 rounded-full h-2 mr-2">
                <div class="bg-blue-500 h-2 rounded-full" style="width: ${Math.min((item.transactions / Math.max(...statusData.map(s => s.transactions))) * 100, 100)}%"></div>
              </div>
